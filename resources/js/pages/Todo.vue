...
import { Todo } from '@/pages/Dashboard.vue';
...
function toggleTodo(): void {
    router.patch(route('todo.toggle', { todo: props.todo.id }));
}
...
<template>
    <div
        :class="[
            'min flex justify-between rounded-lg p-4 shadow transition h-24',
            {
                'bg-gray-50 text-black hover:bg-gray-300': !todo.finished,
                'bg-gray-800 text-white/25 line-through hover:bg-gray-600 border-2 border-white/75': todo.finished,
            },
        ]"
    ...
    >
        ...
        <div class="flex flex-col gap-4">
            <div>{{ todo.name }}</div>
            <div class="text-sm italic">{{ todo.note }}</div>
        </div>

        <!-- Delete button -->
        <button class="hover:scale-125 hover:cursor-pointer" @click.stop="deleteTodo">
            <svg
                :class="['h-8 w-8 transition', { 'fill-white': todo.finished, 'fill-black': !todo.finished }]"
                viewBox="0 0 24 24"
                fill="none"
